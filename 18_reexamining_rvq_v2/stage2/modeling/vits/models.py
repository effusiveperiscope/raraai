...
from ..spk.summarizer import SpeakerSummarizer
...
class SynthesizerTrn(nn.Module):
    def __init__(
        ...
        self.summarizer = SpeakerSummarizer(
            spec_dim=spec_channels,
            hidden_dim=hp.vits.gin_channels,
            summary_dim=hp.vits.spk_dim
        )
        self.dec = MyGeneratorNSF(hp=hp)

    def forward(self, ppg_q, pit, spec, ppg_l, spec_l, sid, pitch_extras=None):
        z_p, m_p, logs_p, ppg_mask, x = self.enc_p(
            ppg_q, ppg_l, f0=f0_to_coarse(pit), pitch_extras=pitch_extras, sid=sid)
        spec_mask = torch.unsqueeze(commons.sequence_mask(spec_l, spec.size(2)), 1).to(
            spec.dtype).to(spec.device)
        summary, s_m, s_logs = self.summarizer(spec, spec_mask)
        g = self.emb_g(summary.unsqueeze(1)).transpose(1,2)
        z_q, m_q, logs_q, _ = self.enc_q(spec, spec_l, g=g)
        ...
        audio = self.dec(
            x=z_slice, f0=pit_slice, 
            spk=summary, pitch_extras=pitch_extras)

        z_f, logdet_f = self.flow(z_q, spec_mask, g=summary)
        z_r, logdet_r = self.flow(z_p, spec_mask, g=summary, reverse=True)

        return audio, ids_slice, spec_mask, \
            (z_f, z_r, z_p, m_p, logs_p, z_q, m_q, logs_q, logdet_f, logdet_r), \
            (s_m, s_logs)

    def posterior_test(self, spec, spec_l, pit, pitch_extras=None):
        spec_mask = torch.unsqueeze(commons.sequence_mask(spec_l, spec.size(2)), 1).to(
            spec.dtype).to(spec.device)
        summary, s_m, s_log = self.summarizer(spec, spec_mask)
        g = self.emb_g(summary.unsqueeze(1)).transpose(1,2)
        z_q, m_q, logs_q, _ = self.enc_q(spec, spec_l, g=g)
        ...
        audio = self.dec(z_slice, pit_slice, spk=summary, pitch_extras=pitch_extras)
        return audio

    def infer(self, ppg_q, pit, ppg_l, spec, spec_l, sid, noise_scale=0.3, pitch_extras=None):
        spec_mask = torch.unsqueeze(commons.sequence_mask(spec_l, spec.size(2)), 1).to(
            spec.dtype).to(spec.device)
        ...
        summary, s_m, s_log = self.summarizer(spec, spec_mask)
        z, _ = self.flow(z_p, ppg_mask, g=summary, reverse=True)
        o = self.dec(z * ppg_mask, spk=summary, f0=pit, pitch_extras=pitch_extras)
        return o

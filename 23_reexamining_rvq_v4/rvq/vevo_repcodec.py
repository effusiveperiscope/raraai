...
class VevoRepCodec(nn.Module):
    ...
    def forward(self, x, do_uq=True):
        ...
        Returns:
            Tuple:
                y (Tensor): Reconstructed output of shape (B, output_channels, T).
                    This is the final decoded signal after encoding, quantization, and decoding.
                zq (Tensor): Quantized latent representation of shape (B, code_dim, T').
                    Latent features where each vector is replaced with the nearest codebook entry.
                z (Tensor): Pre-quantization latent representation of shape (B, code_dim, T').
                    Continuous features output by the encoder + projector before quantization.
                vqloss (Tensor): Vector quantization commitment loss for training.
                    Shape is (num_quantizers,) if multiple quantizers are used.
                perplexity (Tensor): Codebook usage diversity metric. Higher values
                    indicate more uniform code usage. Shape is (num_quantizers,).
...
if __name__ == '__main__':
    ...
    whisper_dim = 512 # whisper base
    codebook_size = 32
    model = VevoRepCodec(
        input_channels=whisper_dim,
        output_channels=whisper_dim,
        encode_channels=whisper_dim,
        decode_channels=whisper_dim,
        code_dim=whisper_dim,
        codebook_num=1,
        codebook_size=codebook_size
    )
    # 14.68 M for whisper_dim=512, codebook_size=32

...
class MelFlowVAEEncoder(torch.nn.Module):
    ...
    def forward(self, x, x_mask):
        ...
        Args:
            x: (batch_size, seq_len, num_mels) - ideally padded to multiple of 16 or downsample rate.
                This is the log mel spectrogram.
...
class MelFlowVAEDecoder(torch.nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, z, z_mask=None, pitch=None):
        ...
        for i in range(len(self.resample_conformers)): # Upsample first
            ...
            x = x * current_mask.unsqueeze(2) # Apply mask before conformer

            # Apply Conformer block
            x = self.resample_conformers[i](x, current_mask)
            x = x * current_mask.unsqueeze(2) # Apply mask after conformer
...
class MelFlowVAE(torch.nn.Module):
    def __init__(self, config):
        super().__init__()
        self.encoder = MelFlowVAEEncoder(config)
        self.decoder = MelFlowVAEDecoder(config)
    ...
    def log_gaussian_prob(self, x, mu, log_var, mask):
    ...
    # Assumes p_z ~ N(0, I)
    def loss(self, x, x_mask, x_recon, z_mean, z_log_var,
         z, z_transformed, log_det, z_mask, kl_beta = 1e-3):
        ...
        loss = recon_loss + kl_beta * kl_div
        return loss, recon_loss, kl_div

if __name__ == '__main__':
    from omegaconf import OmegaConf 
    config = OmegaConf.load("configs/flowvae_f0_v0.yaml")

    net = MelFlowVAE(config)
    #preweights_path = "vae.ckpt"

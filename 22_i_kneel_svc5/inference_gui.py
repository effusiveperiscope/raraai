...
CONFIG = 'configs/char.yaml' # ~! remember to update this

class MainWindow(QMainWindow):
    def __init__(self, config: OmegaConf):
        super().__init__()
        self.config = config
        self.setWindowTitle("22 Inference GUI")
        ...
        gui.addParam(DoubleParam(label="Noise Scale", id='noise_scale', min=0, max=3, default=0.34))
        gui.addParam(DoubleParam(label="Noise Aug Scale", id='noise_aug', min=0, max=3, default=0.0))
        gui.addParam(DoubleParam(label="Intensity Scale", id='intensity_scale', min=0, max=9, default=1.0))
    ...
    def inferAction(self, data: dict):
        ...
        for file in files:
            ...
            if len(prefill_files) > 0:
                # Read file
                wav_data, _ = librosa.load(file, sr=48000)
                # Concatenate
                wav_data = np.concatenate((prefill_data, wav_data))
                # Write to file in memory
                file = io.BytesIO()
                sf.write(file, wav_data, samplerate=48000,
                         format='WAV', subtype='PCM_16')
                file.seek(0)

            wav, sr = librosa.load(file, sr=48000)
            feats = self.my_feats.extract_features(wav, sr)
            ...
            # Truncate
            f0 = f0[:ppg_interp.shape[1]].to(self.dtype).to(self.device).unsqueeze(0)

            f0_confidence = feats['f0_confidence'].to(ppg_interp.dtype).to(self.device)
            f0_subharmonic = feats['f0_subharm'].to(ppg_interp.dtype).to(self.device)
            f0_inharmonic = feats['f0_inharm'].to(ppg_interp.dtype).to(self.device)

            f0_confidence = f0_confidence[:ppg_interp.shape[1]].unsqueeze(0)
            f0_subharmonic = f0_subharmonic[:ppg_interp.shape[1]].unsqueeze(0)
            f0_inharmonic = f0_inharmonic[:ppg_interp.shape[1]].unsqueeze(0)
            ...
            with torch.no_grad():
                o = self.net_g.infer(ppg_interp, vec_interp, f0, spk_feats, ppg_len, pitch_extras=pitch_extras,

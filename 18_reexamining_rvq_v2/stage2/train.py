...
from datatia.actions import RowData
...
class TrainingModule(pl.LightningModule):
    def __init__(self,
        ...
        self.net_d = net_d
        self.codec = codec
        self.test_sample = test_sample # supplies spec and spec_l for testing
        self.config = config
    ...
    def test(self):
        ...
        for j,batch in enumerate(self.test_dataloader):
            with torch.no_grad():
                ...
                test_spec = self.test_sample['spec'].to(self.dtype).to(self.device).unsqueeze(0).transpose(1,2)
                test_spec_len = torch.Tensor([test_spec.shape[2]]).to(self.device)
                ...
                out_audio = self.net_g.infer(ppg_q=ppg_q, pit=f0, ppg_l=ppg_len,
                    spec=test_spec, spec_l=test_spec_len, sid=sid, noise_scale = 
                    self.config.train.get('test_noise_scale', 0.34), pitch_extras=pitch_extras)
                out_audio_post = self.net_g.posterior_test(spec, ppg_len, f0, 
    ...
    def update_stage(self):
        if self.current_epoch < self.config.train.get('stage2_epoch', 0):
            for param in self.net_g.parameters():
                param.requires_grad = False
            for param in self.net_g.enc_p.parameters():
                param.requires_grad = True
            for param in self.net_g.summarizer.parameters():
                param.requires_grad = True
    ...
    def configure_optimizers(self):
        gen_optim = torch.optim.AdamW(
            ...
            weight_decay=self.config.train.weight_decay, eps=1e-7)
    ...
    def step(self, batch, batch_idx, is_train=True):
        ppg = batch['whisper'].to(self.dtype)
        ppg_len = batch['whisper_length']
        ...
        fake_audio, ids_slice, z_mask, \
            ...
            logs_q, logdet_f, logdet_r), (s_m, s_logs) = self.net_g(
                ppg_q, f0, spec, ppg_len, spec_len, sid=sid,
                pitch_extras=pitch_extras)
        ...
        # Mel Loss
        if fake_audio.isnan().any() or audio.isnan().any():
            print("NAN DETECTED")
            return None
        ...
        loss_kl_summary = -0.5 * torch.sum(s_logs + 1 - s_m.pow(2) - s_logs.exp(), dim=1)
        loss_kl_summary = loss_kl_summary.mean() * self.cur_c_kl

        # Loss
        loss_g = score_loss + feat_loss + mel_loss + stft_loss + loss_kl_f + \
            loss_kl_r * 0.5 + loss_kl_summary

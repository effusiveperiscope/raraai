...
from models.model_vae import MelVAE # Assuming model.py exists
...
class VAETrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        if self.config.model.decoder.pitch_conditioning:
            ...
            x, x_mask = batch
            pitch = None
        x_recon, z_mean, z_log_var, z_mask = self.model(x, x_mask, pitch)
        loss, recon_loss, kl_loss = self.model.loss(x, x_mask, x_recon, z_mean, z_log_var, z_mask,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Log images only once per validation epoch (e.g., for batch_idx 0)
        if batch_idx == 0 and self.logger is not None and hasattr(self.logger.experiment, 'add_image'):
             try:
                x_length = x_mask[0].sum().item()
                x0 = x[0][:x_length]
                x_recon0 = x_recon[0][:x_length]
                delta_x = x0 - x_recon0 # Log first item in batch
                delta_x_img = mel_to_img(delta_x.cpu()) # Ensure it's on CPU
                self.logger.experiment.add_image("val/delta_x", delta_x_img, global_step=self.global_step)

                orig_x_img = mel_to_img(x0.cpu())
                self.logger.experiment.add_image("val/original_x", orig_x_img, global_step=self.global_step)

                recon_x_img = mel_to_img(x_recon0.cpu())
                self.logger.experiment.add_image("val/reconstructed_x", recon_x_img, global_step=self.global_step)

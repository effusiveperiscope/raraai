...
class MyFeatures:
    ...
    def init_g2p(self):
        self.g2p = G2PConverter(remove_stress=True, word_boundaries=True, fallback=DefaultG2PFallback())
        self.g2p.load_dictionary("g2p/cmudict.clean", "g2p/new_horsewords.clean")
        self.vocab = OmegaConf.load("g2p/vocab.yaml")['phonemes']
        self.g2p_phones_to_ids = {
            phone: i for i, phone in enumerate(self.vocab)
        }
...
if __name__ == "__main__":
    import librosa
    features = MyFeatures()
    audio, _ = librosa.load("tests/test_voc.mp3", sr=MyFeatures.expected_sample_rate)

...
class VAETrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # log transform
        x = PitchAdjustableMelSpectrogram.dynamic_range_compression_torch(x)
        ...
        loss, recon_loss, kl_loss = self.model.loss(
            x, x_mask, x_recon, z_mean, z_log_var, z, z_transformed, log_det, z_mask, kl_beta=self.kl_beta)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        x_recon, z_mean, z_log_var, z, z_transformed, log_det, z_mask = self.model(x, x_mask, pitch=pitch,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/flowvae_f0_v0.yaml", help="Path to the config file")
    ...
    collator = MelF0Collator(config.train.mel_pad_value, config.model.sampling_ratio)

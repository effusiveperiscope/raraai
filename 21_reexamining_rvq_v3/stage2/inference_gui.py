...
class MainWindow(QMainWindow):
    ...
    def loadCheckpoint(self, checkpoint_name):
        ...
        self.net_i = IntensityModel(in_channels=hp.codec.whisper_dim)
        ...
        load_submodule_prefix(self.net_i, 'net_i.', state, quiet=False)
        self.net_g.to('cuda')
        self.net_g.eval()
    ...
    def inferAction(self, data: dict):
        ...
        for file in files:
            ...
            with torch.no_grad():
                ...
                intensity_feat = intensity_feat * attn * data['intensity_scale']
                intensity = (intensity_feat.sum(dim=1).unsqueeze(1))
                print("int:",intensity.item())

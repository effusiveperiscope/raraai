...
class V10TrainingModule(pl.LightningModule):
    def __init__(self, 
        net_g : V10Synthesizer, 
    ...
    def step(self, batch, batch_idx, is_train=True, is_val=False):
        ...
        y_hat, z_mask, ids_slice, \
            (z, z_p, m_p, logs_p, m_q, logs_q), spk_emb_pred = self.net_g(
                phone=phone_aug, phone_lengths=phone_lengths,
                pitchf=pitchf, y=y_aug, y_lengths=y_lengths,
                ds=sids, lam_grl=lam_grl, pitchq=pitchq)
        ...
        loss_content_inv = F.cosine_embedding_loss(
            spk_emb_pred, spk_emb, torch.ones(spk_emb.shape[0]).to(spk_emb_pred.device))
...
if __name__ == '__main__':
    ...
    training_module = V10TrainingModule(net_g, net_d, config)

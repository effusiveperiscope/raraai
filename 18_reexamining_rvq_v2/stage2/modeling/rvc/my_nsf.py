...
class MyGeneratorNSF(torch.nn.Module):
    def __init__(
        ...
        self.f0_upsamp = torch.nn.Upsample(scale_factor=math.prod(hp.gen.upsample_rates))
        self.m_source = SourceModuleHnNSF(
            sampling_rate=hp.data.sampling_rate, harmonic_num=0, is_half=True
        )
        ...
        for i, (u, k) in enumerate(zip(hp.gen.upsample_rates, hp.gen.upsample_kernel_sizes)):
            ...
            if i + 1 < len(hp.gen.upsample_rates):
                stride_f0 = math.prod(hp.gen.upsample_rates[i + 1 :])
        ...
        for i in range(len(self.ups)):
            ...
            for j, (k, d) in enumerate(
                zip(hp.gen.resblock_kernel_sizes, hp.gen.resblock_dilation_sizes)
            ):
    ...
    def forward(self, x, f0, spk, pitch_extras=None, perturb_scale=0.0):
